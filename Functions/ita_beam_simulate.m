...
%   'solidAngle'    (30)                                    : solid angle of vision, defines the max analysis space
%   'FreqBandwidth' (1/12)                                  : width of the frenquency bands
% 
%
...
% Created:  03-Feb-2009 
%
% Updated by William D'Andrea Fonseca -- Email: will.fonseca@eac.ufsm.br
...
sArgs        = struct('pos1_array','itaMicArray','source',itaMicArray([0.2,0.2,-1],'cart'), ...
                      'type',ita_beam_evaluatePreferences('Method'),'SNR',Inf,'sigmaArray',0, ... 
                      'soundspeed',double(ita_constants('c')),'wavetype',ita_beam_evaluatePreferences('SteeringType'), ...
                      'solidAngle',30,'FreqBandwidth',1/12);
[array,sArgs] = ita_parse_arguments(sArgs,varargin);

%% Body
d = abs(mean(array.z)-mean(sArgs.source.z)); % distance between array and sources
% Maximum scan width for 'solidAngle' degree opening angle
width_max = max(4*max(abs([sArgs.source.x(:).',sArgs.source.y(:).'])),0.1*ceil(10*tan(sArgs.solidAngle*pi/180)*2*d)); 
